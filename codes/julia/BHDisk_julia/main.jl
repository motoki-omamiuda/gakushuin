...
function output()
    savefig(plt, format("./images/test{:2.0d}.png", DEGREE))
end
...
function main()
    ...
    # 像に映る粒子の位置が一部に集中しているため
    # 幅 phi を位置によって変える
    phi_list = []
    phi = 0.0
    while  phi <= pi
        push!(phi_list, phi)
        # phi += 1 / 10 ^ ((1 + cos(phi)^2) * 1.5)
        phi += 1 / 10 ^ ((1 + cos(phi)^2))
    end
    ...
    for r in r_list
        ...
        path = format("./datas/{:2.0f}M_{:1.0f}.csv", r / MASS, n)
